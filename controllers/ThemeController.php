...
{   
    public $page_title = '主题活动';
    public $title_model = 'app\common\models\Theme';
    ...
    public function beforeAction($action)
    {
      ...
      //检查当前单据用户是不是有操作权限
      $admit_allow_arr = ['edit','update','delete','insert','create-contact','create-contract','insert-contact','insert-contract','insert-theme','insert-goods','create-theme','edit-contract','edit-contact','edit-theme'];
    ...
    public function actionEdit($id){
      $this->layout = 'empty';
       ...
       /* foreach ($$ht_list as $key => $value) {
        ...
        return $this->render('create', ['platform'=>$Platform,'id'=>$id,'consignee_list' => $consignee_list,'pg_list' => $pg_list,'contract_list' => $contract_list,'theme_list' => $theme_list]);
    } 
        ...
        if (is_array($goods_id)) {
          ...
          foreach ($goods_id as $key => $value) {
            ...
            if ($goods) {
              ...
              $if_exitd = Yii::$app->db->createCommand("SELECT * FROM goods_platform where goods_id=".$value['goods_id']." and platform_id=".$platform_id)->queryOne();
              if (isset($if_exitd)) {
          ...
          if (isset($goods)) {
            $if_exitd = Yii::$app->db->createCommand("SELECT * FROM goods_platform where goods_id=".$goods_id." and platform_id=".$platform_id)->queryOne();
            if (isset($if_exitd)) {
              $add_goods_error = $goods->goods_name.'已经存在';
            }else {
              //插入数据
