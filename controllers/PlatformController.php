...
class PlatformController extends BaseController
{
    ...
    public function actionDelete($id)
    {
        ...
        if ($Platform) {
            ...
            if ($web_order) {
                message::result_json(2, '项目使用中，不允许删除');
            }
            $Platform->delete();
    ...
    public function actionInsert()
    {
        ...
        if (!$Platform->plat_name) {
            Message::result_json(2, '项目名称不能为空');
        }

        $Platform->add_user_id = Yii::$app->session['manage_user']['id'];
        $Platform->add_user_name = Yii::$app->session['manage_user']['admin_name'];
        $Platform->depart_id = Yii::$app->session['manage_user']['depart_id'];
        $Platform->depart_name = Yii::$app->session['manage_user']['depart_name'];
        $Platform->add_time = time();
