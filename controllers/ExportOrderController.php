...
class ExportOrderController extends BaseController
{
    ...
    public function actionIndex(){
        ...
        return $this->render('index',['init_condition'=>$init_condition,'platform'=>$platform]);
    }
    //删除
    ...
    public function actionInsert(){
        ...
        if (!$ExportOrder->platform_id) {
            Message::result_json(2,'平台不能为空');
        }

        $platform= Platform::findone($ExportOrder->platform_id);
        if (!$platform) {
            Message::result_json(2,'平台不存在，请重新选择');
        }else{
            $ExportOrder->platform_name = $platform->plat_name;
        }
...
//创建出库单据
    public function actionAdd(){
        $this->layout = 'empty';
        $list=Yii::$app->db->createCommand("SELECT * FROM platform order by id desc")->queryAll();
