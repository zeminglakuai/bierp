...
class GoodsController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        if ($supplier_name) {
            ...
            $goods_list = $query->andwhere(['gs.supplier_id' => $supplier_name]);
        }

        if ($purchase_start) {
            $goods_list = $query->andwhere(['>', 'gs.supplier_price', $purchase_start]);
        }

        if ($purchase_end) {
            $goods_list = $query->andwhere(['<', 'gs.supplier_price', $purchase_end]);
        }

        if ($market_start > 0) {
            $goods_list = $query->andwhere(['>', 'g.market_price', $market_start]);
        }

        if ($market_end > 0) {
            $goods_list = $query->andwhere(['<', 'g.market_price', $market_end]);
        }

        if ($sale_start > 0) {
            $goods_list = $query->andwhere(['>', 'g.shop_price', $sale_start]);
        }

        if ($sale_end > 0) {
            $goods_list = $query->andwhere(['<', 'g.shop_price', $sale_end]);
        }
        ...
        if ($export == 1) {
            $goods_list = $query->orderby([$sortby => $order])->groupBy(' g.goods_id')
                ->all();
        } else {
    ...
    public function actionView()
    {
        ...
        if ($goods_id) {
            ...
            $goods_list['goods_platform'] = Yii::$app->db->createCommand("SELECT gp.goods_platform_id, gp.platform_id,gp.startdate,gp.enddate,gp.platform_price, gp.daifa , p.plat_name,p.contract_start_time,p.contract_end_time,p.contract_contact, p.contract_tel FROM goods_platform as gp LEFT JOIN goods as g ON g.goods_id=gp.goods_id LEFT JOIN platform as p ON  p.id =gp.platform_id WHERE gp.goods_id =" . $goods_id)->queryAll();
