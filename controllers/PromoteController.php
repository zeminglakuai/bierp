...
class PromoteController extends BaseController
{
    ...
    public function actionSecondAdmit($id){
      //修改订单状态
      $CustomOrder = CustomOrder::findone($id);
      if ($CustomOrder->custom_order_status <> 1) {
          Message::result_json(2,'当前不允许二次复核!');
      }
      $CustomOrder->admit_user_id = Yii::$app->session['manage_user']['id'];
      $CustomOrder->admit_user_name = Yii::$app->session['manage_user']['admin_name'];
      $CustomOrder->admit_time = time();
      $CustomOrder->custom_order_status = 2;
      $CustomOrder->save(false);
      Message::result_json(1,'复核成功');
    ...
    public function actionCreateAskPrice($id){
      ...
      $custom_order = CustomOrder::findone($id);
      if ($custom_order->custom_order_status < 2) {
      ...
      //$goods_id_list = CustomOrderGoods::find()->asarray()->where(['order_id'=>$id])->all();
      $goods_id_list = ArrayHelper::map($custom_order_goods_list, 'goods_id', 'goods_id');
      if ($goods_id_list) {
        //取得商品的供应商
        $supplier_list = SupplierGoods::find()->where(['in','goods_id',$goods_id_list])->all();

        //按照供应商进行分组
        if ($supplier_list) {
          $supplier_goods_list = [];
          foreach ($supplier_list as $key => $value) {
             $supplier_goods_list[$value->supplier_id][] = $value->goods_id;
          }
          ...
          foreach ($supplier_goods_list as $key => $value) {
            ...
            //插入商品
            $goods_title =['order_id','goods_id','goods_name','goods_sn','isbn','number','market_price','sale_price'];//测试数据键
            $goods_list = (new \yii\db\Query())
                          ->select([$AskPriceOrder->id.' as `order_id`','goods_id','goods_name','goods_sn','isbn','number','market_price','sale_price'])
                          ->from('custom_order_goods')
                          ->andwhere(['order_id'=>$id])
                          ->andwhere(['in','goods_id',$value])
                          ->all();
