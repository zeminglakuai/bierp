<?php
namespace app\modules\web\includes;
...
class Img{  
     ...
     */  
    function thumb($name,$width,$height,$new_name='thumb_img'){  
        //获取图片信息  
        ...
        $newname=$this->SaveNewImage($newimg,$this->path.$new_name,$Info);  
     ...
     * @return boolean 
     */  
    function waterMark($backname,$watername,$waterpos=0,$new_name='new_img') {  
          
        if (file_exists($backname) && file_exists($watername)) {  
            ...
            if(!$pos=$this->getPos($backinfo, $waterinfo, $waterpos)){  
              ...
              $this->SaveNewImage($backimg, $this->path.$new_name, $backinfo);  
        }else{  
    ...
    private function Img_resouce($name,$imageinfo){  
        $iamgeres=$name;
         switch ($imageinfo['type']) {  
            case 'gif':  
              $img=imagecreatefromgif($iamgeres);  
                  break;  
            case 'jpeg':  
              $img=imagecreatefromjpeg($iamgeres);  
                  break; 
			      case 'jpg':  
              $img=imagecreatefromjpeg($iamgeres);  
                  break; 	  
            case 'png':  
              $img=imagecreatefrompng($iamgeres);  
                  break;                          
         };
         return $img;  
    }  
    ...
    private function getImage($imageres,$size,$imageinfo){  
      ...
      private function SaveNewImage($newimg,$newname,$imageinfo){  
        switch ($imageinfo['type']){  
            case 'gif'://gif  
                $result=imagegif($newimg,$newname,85);  
                break;  
            case 'jpeg'://jpg  
                $result=imagejpeg($newimg,$newname,95);  
                break;  
            case 'png'://png  
				
                $result=imagepng($newimg,$newname,85);  
                break;  
        } 
