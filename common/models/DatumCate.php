...
/**
 * This is the model class for table "datum_cate".
 *
 ...
 */
class DatumCate extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['parent_id', 'sort_order','grade','delete_able'], 'integer'],
   ...
   /**
     ...
     * @return  mix
     */
    public function datum_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
    {   
 
        $query = new \yii\db\Query();
        $cat_list  =  $query->select('c.cat_id, c.cat_name, c.parent_id, c.sort_order, COUNT(s.cat_id) AS has_children')
                            ->from('datum_cate as c')
                            ->leftjoin('datum_cate as s','s.parent_id=c.cat_id')
                            ->groupby(['c.cat_id'])
    ...
    */
    ...
    private function get_child_id_list($cat_id){
        if ($cat_id > 0) {
            $query = new \yii\db\Query();
            return $query->select('cat_id')->from('datum_cate')->where(['parent_id'=>$cat_id])->column();
        }else{
            return false;
        }
    }
