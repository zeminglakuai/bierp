...
//            $excel->getActiveSheet()->setCellValue($this->letter[$header_i]."5",isset(lang_config::${$key})?lang_config::${$key}:$value);
        ...
        foreach ($list_data as $key => $value) {
            ...
            if (!empty($value['supplier_id'])) {
                ...
                $data=$supplier->getDataOne($value['supplier_id']);

                $value['supplier_id']=$data['supplier_name'];
                $value['supplier_name']=$data['contact'];
            }
            $j = $i + 6;
            /*向每行单元格插入数据*/
            $col = 0;
            ...
            foreach($title_label as $kk => $vv){
                ...
                /* if (strpos($kk, '&') > 0) {
                   ...
                   $data_label_arr = explode('&', $kk);

                           if ($data_label_arr[1] == 'goods_img') {

                              $is_img = true;
                              $excel->getActiveSheet()->getRowDimension($j) -> setRowHeight(60);

                              $objDrawing = new \PHPExcel_Worksheet_Drawing();
                              $objDrawing->setPath(Yii::getAlias('@app').'/web/'.$value->{$data_label_arr[0]}->$data_label_arr[1]);
