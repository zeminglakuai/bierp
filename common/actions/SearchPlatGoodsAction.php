...
class SearchPlatGoodsAction extends Action {
	public $order_id = 0;
  ...
  $query = new \yii\db\Query();
        $goods_list=$query->select('g.*,gp.*')
        ->from('goods_platform as gp')
        ->leftjoin('goods as g','g.goods_id=gp.goods_id');
		...
		if($search_data['platform_id']){
			$goods_list = $goods_list->andwhere(['gp.platform_id'=>$search_data['platform_id']]);
			$brand = Brand::findone($search_data['platform_id']);
		}
