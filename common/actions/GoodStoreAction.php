...
class GoodStoreAction extends Action {
	public $order_id = 0;

    public function run() {

    	$insert_url   = 'insert-goods';
    	$url          = 'create-goods-store';
    	$supplier_url = 'search-supplier';
    	$search_data = Yii::$app->request->get();

		$search_data['sortby'] = !empty(trim(Yii::$app->request->get('sortby')))?trim(Yii::$app->request->get('sortby')):'goods_id';
		...
		if($search_data['goods_id']){
			$goods_list = $goods_list->andwhere(['goods_id'=>$search_data['goods_id']]);
		}
        if($search_data['type']){
            $goods_list = $goods_list->andwhere(['type'=>$search_data['type']]);
        }

		if($search_data['category']){
			//得到分类的子分类
			$children_cate_id = Category::cat_children_str($search_data['category']); 
			$goods_list = $goods_list->andwhere(['IN','cat_id',$children_cate_id]);
		}

		if($search_data['goods_name']){
			$goods_list = $goods_list->andwhere(['like','goods_name',$search_data['goods_name']]);
		}

		if($search_data['brand_id']){
			$goods_list = $goods_list->andwhere(['brand_id'=>$search_data['brand_id']]);
		...
		if($search_data['supplier']){
			$goods_list = $goods_list->andwhere(['supplier_id'=>$search_data['supplier']]);
			$supplier_info = Supplier::findone($search_data['supplier']);
		}
        if($search_data['purchase_start']){
            $goods_list = $goods_list->andwhere(['>','supplier_price',$search_data['purchase_start']]);
        }

        if($search_data['purchase_end']){
            $goods_list = $goods_list->andwhere(['<','supplier_price',$search_data['purchase_end']]);
        }

		if ($search_data['market_start'] > 0) {
			$goods_list = $goods_list->andwhere(['>','market_price',$search_data['market_start']]);
		}

		if ($search_data['market_end'] > 0) {
			$goods_list = $goods_list->andwhere(['<','market_price',$search_data['market_end']]);
		}

		if ($search_data['sale_start'] > 0) {
			$goods_list = $goods_list->andwhere(['>','shop_price',$search_data['sale_start']]);
		}

		if ($search_data['sale_end'] > 0) {
			$goods_list = $goods_list->andwhere(['<','shop_price',$search_data['sale_end']]);
		}
        ...
        $store=Store::get_store_select();
		// /得到品牌列表
		$brand_list = Brand::get_select_list();
		$this->controller->layout = 'empty';
		return $this->controller->render('/goods/good-store', [
