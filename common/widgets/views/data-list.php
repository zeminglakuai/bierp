					...
					<?php foreach ($data_list as $kk => $vv) { ?>
						<tr id="data_row_<?= $vv->id ?>">
							...
							<td>
								...
								?>
									<A class="btn btn-xs btn-<?= $key == 'delete' ? 'danger' : 'primary'; ?> <?= $key ?>_<?= $model_name_lower ?>" href="<?= $value['type'] == 'js' ? 'javascript:void();' : Url::to(['/' . $model_name_lower . '/' . $value['action'], 'id' => $vv->id]) ?>" data-id="<?= $vv->id ?>" data-id-name="<?= $value['id_name'] ?>" data-toggle="tooltip" data-placement="top" data-original-title="<?= isset($value['tips']) ? $value['tips'] : '' ?>" data-action="<?= $value['action'] ?>">
