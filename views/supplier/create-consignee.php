...
$("#add_consignee").click(function(){
    var formData = new FormData($( "#data_form" )[0]);
     $.ajax({  
          url: '<?= Url::to(["supplier/insert-consignee","id"=>$id])?>' ,  
          type: 'POST',  
          data: formData,
          dataType:'json',
          async: false,  
          ...
          success: function (result) {  
            if(result.error == 1){
               parent.location.href= '<?= Url::to(["supplier/edit","id"=>$id,"present_panel"=>"consignee"])?>';
            }else{
...
$("#edit_consignee").click(function(){
    var formData = new FormData($( "#data_form" )[0]);
     $.ajax({  
          url: '<?= Url::to(["supplier/update-consignee","id"=>$id,"consignee_id"=>$consignee->id])?>',
          type: 'POST',  
          data: formData,
          dataType:'json',
          async: false,  
