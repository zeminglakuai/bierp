...
<?= app\common\widgets\Submit::widget(['model'=>$contact,'model_name'=>"platform_contact",'form_name'=>'data_form','defined_function'=>true]); ?>
<script type="text/javascript">

$("#add_platform_contact").click(function(){
    var formData = new FormData($( "#data_form" )[0]);
     $.ajax({  
          url: '<?= Url::to(["platform/insert-contact","id"=>$id])?>' ,  
          type: 'POST',  
          data: formData,
          dataType:'json',
          async: false,  
...
$("#edit_platform_contact").click(function(){
    var formData = new FormData($( "#data_form" )[0]);
     $.ajax({  
          url: '<?= Url::to(["platform/update-contact","id"=>$id,"contact_id"=>$contact->id])?>',
          type: 'POST',  
          data: formData,
          dataType:'json',
          async: false,  
