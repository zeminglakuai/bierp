...
<?= app\common\widgets\OrderTitle::widget([
                      ...
                      'id'=>$id,
                      'model_name'=>'purchse-contract',
                      'label_arr' => ['contract_name'=>'',
                                      'supplier_name'=>'',
                                      'purchase&order_sn'=>['link'=>true,'id'=>'purchase','url'=>'/purchase/view','id'=>'id'],
                                      'template_name'=>'',
                                      'keeper_user_name'=>'',
                                      'keep_time'=>'',
                                      'remark'=>''
    ...
    <div class="row">
      <div class="col-sm-12">
        <form action="" id="data_form">
          <?= app\common\widgets\FileList::widget(['model'=>'purchase-contract','file_list'=>$contract->contractFile]); ?>
        </form>
      </div>
    </div>
    <div class="row">
...
<script type="text/javascript">
  $("#contract_file").click(function(){
   ...
   var update_url = create_url('<?= Url::to(["/purchase-contract/contract-file"]) ?>');
     $.ajax({  
...
<script type="text/javascript">
  ...
  $(".contract_editable").click(
    function(){
      ...
      $('.edit_input').blur(function(){
        ...
        $.get('/purchase-contract/update-label',{value: value, id:id, data_name: data_name},function(result){
          if(result.error == 1){
  ...
  $("#edit_contract").click(function(){
    ...
    layer.open({
      ...
      area: ['90%', '80%'], //宽高
      content: "<?= Url::to(['purchase-contract/edit-contract','id'=>$contract->id])?>"
    });
  });
  ...
  $("#keep_contract").click(function(){
    ...
    layer.open({
      ...
      //skin: 'layui-layer-rim', //加上边框
      area: ['90%', '80%'], //宽高
      content: "<?= Url::to(['purchase-contract/keep-contract','id'=>$contract->id])?>"
    });
  });  
